...
### TAREA: Prompt de sync para ChatGPT — contenido/redes + pregunta sobre Cloudflare Web Analytics/Zaraz
**OBJETIVO:** Preparar un prompt listo para pegar en ChatGPT que le dé el contexto real (sin datos inventados) para pensar el calendario de contenido inicial, la estructura de UTMs, y su opinión razonada sobre si conviene usar Cloudflare Web Analytics/Zaraz (plan Free) junto a GA4 — a pedido explícito del usuario tras la auditoría de la entrada anterior.
**ESTADO:** Completado (documento). Sin cambios de código ni de infraestructura.
**ARCHIVOS MODIFICADOS:** `docs/PROMPT_CHATGPT_CONTENIDO_REDES_2026-09-09.md` (nuevo). Sin cambios en `index.html` ni en `PRICING`.
**CONTENIDO:** El prompt resume los 3 vacíos ya documentados (sin calendario/historial, audiencia insuficiente, sin UTMs para medir conversión de redes en GA4), lista los activos reales disponibles, aclara que ChatGPT no tiene acceso a los archivos reales y que cualquier propuesta suya pasa por revisión de Claude antes de aplicarse (regla ya vigente en la sección 11 de este archivo), y pide explícitamente que no invente fotos, videos ni cifras.
**PROBLEMAS DETECTADOS:** Ninguno.
